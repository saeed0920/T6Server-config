...
updateDevSettings()
{
	...
	// these two dvars rely on dev-only functions from other files
	...
	if ( getDvar("scr_suicide") != "")
	{
		thread commitSuicide( getDvar("scr_suicide") );

		SetDevDvar( "scr_suicide", "" );
	}
...
xKillsY( attackerName, victimName )
{
	...
	victim thread [[level.callbackPlayerDamage]](
		...
		"iw5_scar_mp", // sWeapon The weapon number of the weapon used to inflict the damage
...
commitSuicide( playerName )
{
	player = undefined;

	for ( index = 0; index < level.players.size; index++ )
	{
		if ( level.players[index].name == playerName )
